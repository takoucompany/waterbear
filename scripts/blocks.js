...
function choice_func(s, listname, default_opt){
	...
	if (typeof callback_lists === "undefined" || typeof callback_lists[listname] === "undefined") {
	    ...
	    var callback = callback_lists[listname];
	    return '<span class="value string ' + listname + ' autosocket" data-type="  "><select '+'onchange="' + callback + '(this)">' +
	        list.map(function(item){
	            if (item === default_opt){
	                return '<option selected>' + item + '</option>';
	            }else{
